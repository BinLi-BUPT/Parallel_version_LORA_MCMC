...
def Rotation_estimation(samples):
    ...
    samples_Cov = (samples_mean0.dot(samples_mean0.T)) / samples_num
...
def get_grid_points_all(xMin, xMax, N, RN, R=False):
    ...
    with multiprocessing.Manager() as manager:
        ...
        time_list = manager.list([0] * len(args_list))

        for i in range(len(args_list)):
            args = args_list[i]
            args_list[i] = (args, grid_points_all_list, time_list, id)
            id += 1
        ...
        time_parallel = min([i for i in list(time_list) if i != 0])

        time_4 = time.time()

        # Since the initialization of grid_points can be completed before the program runs, the time for this part is not included in the program.
        total_time = 0

        # However, you can include this part in the program's runtime by uncommenting the following line of code.
        # total_time = (time_2 - time_1) + time_parallel + (time_4 - time_3)

    return grid_points_all, total_time


def run_00(args, grid_points_all_list, time_list, id):
    time_1 = time.time()
    ...
    grid_points_all_list[id] = grid_points
    time_list[id] = time_2 - time_1

    return 0


def Mutlimodal_density_separation(fun, xMin, xMax, N, RN, des1, des2, num_node, num_cpu):
    grid_points_all, time_parallel_1 = get_grid_points_all(xMin, xMax, N, RN)

    X, time_parallel_2 = fun_01(grid_points_all, fun, des1, num_node, num_cpu)
    ...
    total_time = time_parallel_1 + time_parallel_2 + (time_2 - time_1)
...
def fun_01(grid_points_all, fun, des1, num_node, num_cpu):
    ...
    for node_index in range(num_node):
        ...
        for cpu_index in range(num_cpu):
            cpu_args = (cpu_grid_points_list[node_index][cpu_index], fun, des1, id)
...
def run_01_on_node_{}(args_list, num_cpu):  
    ...
    with multiprocessing.Manager() as manager:
        ...
        for i in range(len(args_list)):
            grid_points_list, fun, des1, id = args_list[i]
            args_list[i] = (grid_points_list, fun, des1, X_dict, time_dict, id)
            id_list.append(id)
        ...
        for id in id_list:
            X.extend(X_dict[str(id)])
            time_list.append(time_dict[str(id)])
        
        time_parallel = min(time_list)
...
def run_01(grid_points_list, fun, des1, X_dict, time_dict, id):
    time_1 = time.time()
    ...
    X_dict[str(id)] = X
    time_dict[str(id)] = time_2 - time_1
...
def Local_region(fun, Rot, xMin, xMax, N, RN, des1, des2, num_node, num_cpu):
    ...
    X, time_parallel_2 = fun_02(grid_points_all, Rot, fun, des1, num_node, num_cpu)
...
def fun_02(grid_points_all, Rot, fun, des1, num_node, num_cpu):
    ...
    for node_index in range(num_node):
        ...
        for cpu_index in range(num_cpu):
            cpu_args = (cpu_grid_points_list[node_index][cpu_index], Rot, fun, des1, id)
            node_args.append(cpu_args)
            id += 1
...
def run_02_on_node_{}(args_list, num_cpu):
    ...
    with multiprocessing.Manager() as manager:
        ...
        for i in range(len(args_list)):
            grid_points_list, Rot, fun, des1, id = args_list[i]
            args_list[i] = (grid_points_list, Rot, fun, des1, X_dict, time_dict, id)
            id_list.append(id)
...
def run_02(grid_points_list, Rot, fun, des1, X_dict, time_dict, id):
    time_1 = time.time()
...
def Reconstruct_lowrank_proposal(fun, Rot, xMin, xMax, N, RN, num_node, num_cpu):
    ...
    grid_points_all, time_parallel_1 = get_grid_points_all(xMin, xMax, N, RN, R=True)

    C_pr, time_parallel_2 = fun_03(grid_points_all, Rot, fun, num_node, num_cpu)
    ...
    total_time = (time_2 - time_1) + time_parallel_1 + time_parallel_2 + (time_4 - time_3)
...
def fun_03(grid_points_all, Rot, fun, num_node, num_cpu):
    ...
    for node_index in range(num_node):
        ...
        for cpu_index in range(num_cpu):
            cpu_args = (cpu_grid_points_list[node_index][cpu_index], Rot, fun, id)
            node_args.append(cpu_args)
            id += 1
...
def run_03_on_node_{}(args_list, num_cpu):
    ...
    with multiprocessing.Manager() as manager:
        ...
        for i in range(len(args_list)):
            grid_points_list, Rot, fun, id = args_list[i]
            args_list[i] = (grid_points_list, Rot, fun, C_pr_dict, time_dict, id)
            id_list.append(id)
...
def run_03(grid_points_list, Rot, fun, C_pr_dict, time_dict, id):
    time_1 = time.time()
...
def fun_04(samples_num, D, C, ind1, U, R, Xss, Xs_interr, N_interr, ind_inter1, x1, gaps_interr, Xs_minn, gapss, N, Rot,
    ...
    for node_index in range(num_node):
        ...
        for cpu_index in range(num_cpu):
            cpu_args = (
                cpu_s_list[node_index][cpu_index], D, C, ind1, U, R, Xss, Xs_interr, N_interr, ind_inter1, x1,
                gaps_interr, Xs_minn, gapss, N, Rot, fun, p, xo)
            node_args.append(cpu_args)
...
def run_04_on_node_{}(args_list, samples_num, num_cpu):
    ...
    with multiprocessing.Manager() as manager:
        ...
        for i in range(len(args_list)):
            ...
            args_list[i] = (s_list, D, C, ind1, U, R, Xss, Xs_interr, N_interr, ind_inter1, x1, gaps_interr, Xs_minn, gapss, N, Rot, fun, p, xo, samples_dict, samples_p_dict, samples_alphas_dict, samples_rot_dict, time_dict, id)
...
res_{} = run_04_on_node_{}.remote(args_list[{}], samples_num, num_cpu)
    ...
    for node in range(num_node):
        code = """
res_{} = ray.get(res_{})
samples_list.extend(res_{}[0])
samples_p_list.extend(res_{}[1])
samples_alphas_list.extend(res_{}[2])
samples_rot_list.extend(res_{}[3])
time_list.append(res_{}[4])
        """.format(node + 1, node + 1, node + 1, node + 1, node + 1, node + 1, node + 1)
        exec(code)
...
def run_04(s_list, D, C, ind1, U, R, Xss, Xs_interr, N_interr, ind_inter1, x1, gaps_interr, Xs_minn, gapss, N, Rot, fun,
           p, xo, samples_dict, samples_p_dict, samples_alphas_dict, samples_rot_dict, time_dict, id):
    time_1 = time.time()
    ...
    for s in s_list:
        ...
        total_time += time_4 - time_3

    time_dict[str(id)] = (time_2 - time_1) + total_time

    return 0


def Global_alignment(all_samples, all_P, samples_num, samples_num2, w_lims, num_node, num_cpu):
    time_1 = time.time()
    ...
    mean_alphas, time_parallel = fun_05(NW, W1, W2, all_samples, all_P, all_Q, samples_num, num_node, num_cpu)
...
def fun_05(NW, W1, W2, all_samples, all_P, all_Q, samples_num, num_node, num_cpu):
    time_1 = time.time()
    ...
    for i in range(NW):
        ...
        for j in range(NW):
            args = (best_w_temp_0, W2, j, all_samples, all_P, all_Q, samples_num, id)
            args_list.append(args)
            id += 1

    args_list, _ = split_list(args_list, num_node)
    ...
    for node in range(num_node):
        code = """
@ray.remote(num_cpus=num_cpu, resources={{"node:RayNode{}": 1}})
def run_05_on_node_{}(args_list, num_cpu):  
    time_1 = time.time()
    ...
    with multiprocessing.Manager() as manager:
        ...
        for i in range(len(args_list)):
            best_w_temp, W2, j, all_samples, all_P, all_Q, samples_num, id = args_list[i]
            args_list[i] = (best_w_temp, W2, j, all_samples, all_P, all_Q, samples_num, mean_alphas_dict, time_dict, id)
            id_list.append(id)

        time_2 = time.time()

        for args in args_list:
            process = Process(target=run_05, args=args)
            processes.append(process)
            process.start()
        ...
        for id in id_list:
            mean_alphas.append(mean_alphas_dict[str(id)])
            time_list.append(time_dict[str(id)])

        time_parallel = min(time_list) * (len(args_list) / num_cpu)

        total_time = (time_2 - time_1) + time_parallel

    return mean_alphas, total_time
        """.format(node + 1, node + 1)
        exec(code)
    ...
    for node in range(num_node):
        code = """
res_{} = run_05_on_node_{}.remote(args_list[{}], num_cpu)
        """.format(node + 1, node + 1, node)
        exec(code)
...
time_list.append(res_{}[1])
    ...
    mean_alphas = np.array(mean_alphas).reshape((NW, NW))
...
def run_05(best_w_temp_0, W2, j, all_samples, all_P, all_Q, samples_num, mean_alphas_dict, time_dict, id):
    time_1 = time.time()

    best_w_temp_1 = W2[j]
    best_w_temp_2 = 1 - best_w_temp_0 - best_w_temp_1

    if best_w_temp_0 + best_w_temp_1 >= 1:
        mean_alpha_val = 0

    else:
        best_w_temp = np.array((best_w_temp_0, best_w_temp_1, best_w_temp_2))
        _, mean_alpha_val, _, _ = Generation_of_Gobal_chain(all_samples, all_P, all_Q, samples_num, best_w_temp)

    time_2 = time.time()

    mean_alphas_dict[str(id)] = mean_alpha_val
    time_dict[str(id)] = time_2 - time_1
