...
def LORA(fun, xMin, xMax, samples_num, N, RN, D, num_node, num_cpu):
    # ALGORITHM PROCESS:

    ### Step1. Automatic identification and separation of multiple well-separated local modes.
    Rot1, xMin1, xMax1, cluster_centre, cluster_centre_rot, time_part_1 = LORA_splite_serial(fun, xMin, xMax, N, RN, 1e-10, 1e-10, num_node, num_cpu)

    ### Step2. For each local mode, estimating the rotation direction of coordinate system for more effective sampling.
    xMin2_1, xMax2_1, Rot2_1, time_part_2_1 = Local_region(fun, Rot1, xMin1[0], xMax1[0], N, RN, 1e-8, 1e-7, num_node,
                                                           num_cpu)
    xMin2_2, xMax2_2, Rot2_2, time_part_2_2 = Local_region(fun, Rot1, xMin1[1], xMax1[1], N, RN, 1e-8, 1e-7, num_node,
                                                           num_cpu)
    xMin2_3, xMax2_3, Rot2_3, time_part_2_3 = Local_region(fun, Rot1, xMin1[2], xMax1[2], N, RN, 1e-8, 1e-7, num_node,
                                                           num_cpu)

    time_part_2 = time_part_2_1 + time_part_2_2 + time_part_2_3

    # Step3. Learning the representative proposal for each local mode.
    C_pr1, R1, c_ind1, time_part_3_1 = Reconstruct_lowrank_proposal(fun, Rot2_1, xMin2_1, xMax2_1, N, RN, num_node, num_cpu)
    C_pr2, R2, c_ind2, time_part_3_2  = Reconstruct_lowrank_proposal(fun, Rot2_2, xMin2_2, xMax2_2, N, RN, num_node, num_cpu)
    C_pr3, R3, c_ind3, time_part_3_3  = Reconstruct_lowrank_proposal(fun, Rot2_3, xMin2_3, xMax2_3, N, RN, num_node, num_cpu)

    time_part_3 = time_part_3_1 + time_part_3_2 + time_part_3_3
    ...
    samples_inf_1, time_part_4_1 = Sampling_in_interpreted_space(init_x, C, U, R, fun, xMin, xMax, Rot, N, samples_num,
                                                                 2, num_node, num_cpu)
    ...
    time_part_4_1 += time_2 - time_1 + time_4 - time_3
    ...
    samples_inf_2, time_part_4_2 = Sampling_in_interpreted_space(init_x, C, U, R, fun, xMin, xMax, Rot, N, samples_num,
                                                                 2, num_node, num_cpu)
    ...
    time_part_4_2 += time_2 - time_1 + time_4 - time_3
    ...
    samples_inf_3, time_part_4_3 = Sampling_in_interpreted_space(init_x, C, U, R, fun, xMin, xMax, Rot, N, samples_num,
                                                                 2, num_node, num_cpu)
    ...
    time_part_4_3 += time_2 - time_1 + time_4 - time_3

    time_part_4 = time_part_4_1 + time_part_4_2 + time_part_4_3
    ...
    time_list = [total_time, time_part_1, time_part_2, time_part_3, time_part_4, time_part_5]
...
def main():
    ...
    num_node_list = [1, 5, 10, 15, 20]
    num_cpu_list = [1, 4, 8, 12, 16]

    num_node_list = [20]
    num_cpu_list = [16]

    main_time_list = [0, 0, 0, 0, 0, 0]
    main_res = 0
        ...
        for num_node in num_node_list:
            for num_cpu in num_cpu_list:
                for run in range(total_iters):
                    ...
                    _, _, _, _, time_list, res = LORA(fun, xMin, xMax, samples_num, N, RN, D, num_node, num_cpu)

                    file.write("total_time_cost\t\ttime_cost_1\t\ttime_cost_2\t\ttime_cost_3\t\ttime_cost_4\t\ttime_cost_5:\n")
